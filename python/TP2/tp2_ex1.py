...
# le temps d'exécution de fibo_1 croît beaucoup plus
# rapidement que celle de fibo2 et celle de fibo3
# croît encore plus vite (proche de la courbe n**2)
...
# 
# Calcule
#    le n-ieme terme de la suite de Fibonacci et 
#    le nombre d'additions de (grands) entiers utilisees

# A COMPLETER
def fibo1_bis(n):
  if n <= 2 : return 0
  return 1 + fibo1_bis(n-1) + fibo1_bis(n-2) 
def fibo_1_adds(n) :
  op = fibo1_bis(n)
  x = fibo_1(n)
  return x, op
...
def courbes_adds(n, tous=True, pas=1) :
  ''' affiche les courbes des additions effectuees pour le calcul de Fn par les differents algos
  (les trois si tous=True, valeur par defaut; seulement fibo_2 et fibo_3 si tous=False)'''
...
def fibo_1_bits(n) :
  ...
  x = fibo_1_bits(n-1)
  y = fibo_1_bits(n-2)
  return nbOfBits(x[0] + y[0]) + x[0] + y[0], x[1] + y[1]
...
###############################################################################################
###############################################################################################
########################### courbes - operations sur les bits  ################################
...
def courbes_ops(n, tous=True, pas=1) :
  ''' affiche les courbes des operations élémentaires effectuées pour le calcul de Fn par les différents algos '''
...
if __name__ == '__main__':
  ...
  test_fibo_bits(2)
  test_fibo_bits(3)
  # courbes du nombre d'operations elementaires sur les bits
